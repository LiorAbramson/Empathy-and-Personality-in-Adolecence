...
*the examination of missingness effects of parental reports is conducted below, in the end of the script
```{r}
...
for cognitive empathy only: 
creating residual scores cleaned also from source (pencil/online) and attrition (one/two time points)
for a robustness check (Appendix 4)
*notice that these variables were not entered into the final Mx data file. 
*Change the script accordingly if you want to include them 
```{r}
...
*notice that after running this code and before entering the file to the Mx software, you need to do the following (outside of R):
1) erase the columns names
2) erase the ifam column (Mx doesn't process letters input)
3) turn NA values into 999 
4) name the file 'DWide_4Mx_noifams'
5) convert the file into a .dat format
```{r}
  col <- colnames(DWide)
  ...
  write.csv(DWide_4Mx, file= "~/Documents/projects/Empathy-Personality-Adolecence/OSF/data/DWide_4Mx.csv", row.names = F)         
```

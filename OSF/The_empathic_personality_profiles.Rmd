...
#Preparing the file (change the directory to the relevant path on your computer)
  D <- read.csv("~/Documents/projects/Empathy-Personality-Adolescence/OSF/data/Age11_EmpPer_anonymized.csv")
...
```{r}
#preparing the file (change the directory to the relevant path on your computer)
  D13 <- read.csv ("~/Documents/projects/Empathy-Personality-Adolescence/OSF/data/Age13_EmpPer_anonymized.csv")
...
#we note that in terms of the correlations and MSEs mean and range, the results are similar regardless of the code you use.  
  
#code for exact replication  
#  Age11_gfolds <- read.csv("~/Documents/projects/Empathy-Personality-Adolescence/OSF/data/Age11_gfold.csv")
#  Age13_gfolds <- read.csv("~/Documents/projects/Empathy-Personality-Adolescence/OSF/data/Age13_gfold.csv")
...
#import data (called panel sample)
  DPan <- read.csv ("~/Documents/projects/Empathy-Personality-Adolescence/OSF/data/panel_results_PerEmp.csv")
...
IMPORTANT NOTE: The permutation function builds the null hypothesis distribution by randomly scrambling the empathy variables between families. This procedure relies on participants ID numbers. Since IRB restrictions and confidentiality considerations prevent us from uploading the original ID numbers, the random scrambling that was used in the paper cannot be exactly replicated with the new numbers. Specifically, with the new numbers, all results remain the same, except for the significance level of two item coefficients in the age 11-cognitive empathy analysis.
#If you want to replicate the paper significance testing results to the exact number, contact the first author- Lior Abramson, at labramson87@gmail.com. Depending on IRB approval, we will provide you with the key to the original participants' ID numbers.
```{r}

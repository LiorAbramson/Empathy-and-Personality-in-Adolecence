...
#the random division to folds relies on participants ID numbers. Since IRB restrictions and confidentiality considerations
#prevent us from uploading the original ID numbers, the fold division that was used in the study cannot be replicated with the above code.
#if you want to exactly replicate the results of the study, you will have to merge the original fold division from an external file. To do so, use the following code INSTEAD of the row above (remove the # before).
  
#  Age11_gfolds <- read.csv("~/Documents/projects/Empathy-Personality-Adolecence/OSF/data/Age11_gfold.csv")
#  DImp1.1<- merge(DImp1.1,Age11_gfolds[,c("ifam","ID","gfold")], by=c("ifam","ID"), all.x = T, all.y = F) 
